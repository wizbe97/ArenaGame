...
public class NetworkManager : MonoBehaviour, INetworkRunnerCallbacks
{
	...
	public async void StartHost()
	{
		...
		var spawnedObj = _runner.Spawn(lobbyManagerPrefab, Vector3.zero, Quaternion.identity, _runner.LocalPlayer);
		var lobby = spawnedObj.GetComponent<LobbyManager>();
		lobby.playerGrid = playerGrid;
		lobby.RefreshLobbyUI();

		lobby.RegisterPlayer(_runner.LocalPlayer);
	...
	public async void JoinLobby(string steamID)
	{
		...
		Debug.Log($"🔵 Attempting to join lobby with Steam ID: {steamID}");
